...
longCombat <- function(idvar,
  ...
  if (sum(is.na(data)) > 0) {
    missing <- paste(names(data)[apply(data, 2, function(x) sum(is.na(x)) > 0)],
                     collapse = ", ")
    message <- paste0(
      "Missing data in variables:\n\n",
      missing,
      "\n\nBefore running longCombat either impute the missing values, remove those rows, or remove columns with missing values if that variable is not in the model." # nolint
    )
    stop(message)
  }
  ...
  if (min(table(batch)) <= 1) {
    batch_single <- paste(names(table(batch))[table(batch) <= 1], collapse =
                            ", ")
    message <- paste0(
      "The following batches have only one observation:\n\n",
      batch_single,
      "\n\nlongCombat needs at least 2 observations per batch to harmonize variance across batch. Remove rows for these batches before running longCombat." # nolint
    )
    stop(message)
  }
  if (verbose)
    cat("[longCombat] found", nlevels(batch), "batches\n")
  # number of batches
  m <- nlevels(batch)
  ...
  if (verbose)
    cat("[longCombat] found", V, "features\n")
  # total number of observations
  L <- nrow(data)
  if (verbose)
    cat("[longCombat] found", L, "total observations\n")
  
  ##############################-
  # standardize data across features
  ##############################-
  if (verbose)
    cat("[longCombat] standardizing data across features...\n")
  ...
  for (v in 1:V) {
    ...
    if (verbose)
      cat(paste0("[longCombat] fitting lme model for feature ", v, "\n"))
    # make the linear mixed effects model lmer formula
    lme_formula <- as.formula(paste0(featurenames[v], "~",
    ...
    if (method == "REML") {
      ...
      sigma_estimates[v] <- corr_estimates[corr_estimates$grp == "Residual",
                                           "sdcor"]
    } else if (method == "MSR") {
      resid <- residuals(lme_fit)
      sigma_estimates[v] <- sqrt((sum((
        resid - mean(resid)
      ) ^ 2) / length(resid)))
    }
    # save batch effects
    batch_effects[, v] <- lme4::fixef(lme_fit)[
      grep(batchvar, names(lme4::fixef(lme_fit)))
    ]
    # save predicted values
    predicted[, v] <- fitted(lme_fit)
  } # end loop over features
  # create a L*V matrix of sigma estimates
  sigmas <- matrix(rep(sigma_estimates, each = L),
                   nrow = L,
                   ncol = V)
  ...
  for (i in 1:m) {
    ...
    batch_effects_expanded[batches[[i]], ] <-
      matrix(rep(batch_effects_adjusted[i, ], length(batches[[i]])),
             ncol = V,
             byrow = TRUE)
  } # end loop over batches
  # standardize the data
  data_std <-
    (data[, featurenames] - predicted + batch_effects_expanded) / sigmas
  
  ##############################-
  # method of moments to estimate hyperparameters
  ##############################-
  if (verbose)
    cat("[longCombat] using method of moments to estimate hyperparameters\n")
  ...
  ##############################-
  if (verbose)
    cat("[longCombat] using empirical Bayes to estimate batch effects...\n")
  if (verbose)
    cat("[longCombat] initializing...\n")
  # get initial estimates
  gammastarhat0 <- matrix(nrow = m, ncol = V)
  for (v in 1:V) {
    # begin loop over features
    gammastarhat0[, v] <-
      ((ni * tau2bar * gammahat[, v]) + (delta2hat[, v] * gammabar)) /
      ((ni * tau2bar) + delta2hat[, v])
  } # end loop over features
  ...
  for (v in 1:V) {
    ...
    for (i in 1:m) {
      # begin loop over batches
      zminusgammastarhat2 <- sum((data_std[batches[[i]], v] -
                                    gammastarhat0[i, v]) ^2)
      delta2starhat0[i, v] <- (thetabar[i] + 0.5 * zminusgammastarhat2) /
        (ni[i] / 2 + lambdabar[i] - 1)
    } # end loop over features
  } # end loop over batches
  # iterate
  ...
  for (b in 2:(niter + 1)) {
    ...
    if (verbose)
      cat(paste0("[longCombat] starting EM algorithm iteration ",
                 (b - 1),
                 "\n"))
    for (v in 1:V) {
      # begin loop over features
      gammastarhat[, v, b] <-
        ((ni * tau2bar * gammahat[, v]) +
         (delta2starhat[, v, (b - 1)] * gammabar)) /
        ((ni * tau2bar) + delta2starhat[, v, (b - 1)])
      
      for (i in 1:m) {
        # begin loop over batches
        zminusgammastarhat2 <-
          sum((data_std[batches[[i]], v] - gammastarhat[i, v, (b - 1)]) ^ 2)
        
        delta2starhat[i, v, b] <-
          (thetabar[i] + 0.5 * zminusgammastarhat2) /
          (ni[i] / 2 + lambdabar[i] - 1)
      } # end loop over batches
    } # end loop over features
  ...
  ##############################-
  if (verbose)
    cat("[longCombat] adjusting data for batch effects\n")
  ...
  for (i in 1:m) {
    # loop over batches
    gammastarhat_expanded[batches[[i]], ] <-
      matrix(rep(gammastarhat_final[i, ], length(batches[[i]])),
             ncol = V,
             byrow = TRUE)
    delta2starhat_expanded[batches[[i]], ] <-
      matrix(rep(delta2starhat_final[i, ], length(batches[[i]])),
             ncol = V,
             byrow = TRUE)
  } # end loop over batches
  # do ComBat
  data_combat <- (sigmas / sqrt(delta2starhat_expanded)) *
    (data_std - gammastarhat_expanded) + predicted - batch_effects_expanded
  ...
  # add names
  colnames(data_combat) <- c(idvar, timevar, batchvar,
                             paste0(featurenames, ".combat"))
