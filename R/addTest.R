...
addTest <- function(idvar,
  ...
  if (verbose)
    cat("[addTest] found", nlevels(batch), "batches\n")
  # feature names
  ...
  for (v in 1:V) {
    ...
    if (verbose)
      cat("[addTest] testing for additive batch effect for feature ",
          v,
          "\n")
    # create full and reduced model formulas
    # full model includes a batch fixed effect
    full_formula <- as.formula(paste0(featurenames[v], "~",
                                      formula, "+", batchvar, "+", ranef))
    # reduced model omits a batch fixed effect
    reduced_formula <- as.formula(paste0(featurenames[v], "~",
                                         formula, "+", ranef))
    ...
    # save in results
    addEffect[v, ] <- c(featurenames[v],
                        KR$stats$Fstat,
                        KR$stats$ddf,
                        KR$stats$p.value)
  } # end loop over features
  # sort according to KR F statistic
  addEffect_ordered <- addEffect[order(as.numeric(addEffect$KRFstat), decreasing =
                                         TRUE), ]
  # add column names
  colnames(addEffect_ordered) <- c("Feature",
                                   paste0("KR F(", KR$stats$ndf, ", KRddf)"),
                                   "KRddf",
                                   "KR p-value")
  # return result
