...
multTest <- function(idvar,
  ...
  if (verbose)
    cat("[multTest] found", nlevels(batch), "batches\n")
  # feature names
  ...
  for (v in 1:V) {
    ...
    if (verbose)
      cat("[multTest] testing for multiplicative batch effect for feature ",
          v,
          "\n")
    ...
    # save residuals
    fit_residuals <- residuals(fit_full) # nolint
    # do Fligner-Killeen test
    FKtest <- fligner.test(fit_residuals ~ batch)
    # save results
    multEffect[v, ] <- c(featurenames[v],
                         FKtest$statistic,
                         FKtest$parameter,
                         FKtest$p.value)
  } # end loop over features
  # sort according to FK chi-squared statistic
  multEffect_ordered <- multEffect[order(as.numeric(multEffect$Chisq),
                                         decreasing = TRUE), ]
  # add column names
