...
trajPlot <- function(idvar,
  ...
  # print messages
  if (verbose)
    cat("[trajPlot] found", n, "unique subjects\n")
  if (verbose)
    cat("[trajPlot] found", L, "observations\n")
  if (verbose)
    cat("[trajPlot] found", m, "batches\n")
  ...
  if (is.null(point.shape))
    point.shape <- as.numeric(data[, batchvar]) %% 26
  if (is.null(point.col))
    point.col <- rep("black", L)
  if (is.null(line.col))
    line.col <- rep("black", n)
  if (is.null(line.type))
    line.type <- rep("solid", n)
  ...
  points(
    data.subject[, timevar],
    data.subject[, feature],
    pch = data.subject$point.shape,
    col = data.subject$point.col
  )
  for (i in 2:n) {
    # begin loop over remaining subjects
    # get the data
    data.subject <- data[data[, idvar] == subjectIDs[i], ]
    # sort by time point
    data.subject <- data.subject[order(data.subject[, timevar]), ]
    # plot line
    lines(data.subject[, timevar],
          data.subject[, feature],
          col = line.col[i],
          lty = line.type[i])
    # plot points
