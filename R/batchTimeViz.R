...
batchTimeViz <- function(batchvar,
                         timevar,
                         data,
                         xlabel = "time",
                         ylabel = "batch",
  ...
  if (verbose)
    cat("[batchViz] found", nlevels(batch), "batches\n")
  # number of batches
  m <- nlevels(batch)
  ...
  for (i in 1:m) {
    ...
    # get duration
    batchtime_duration[i] <- max(data[batches[[i]], timevar]) -
      min(data[batches[[i]], timevar])
  } # end loop over batches
  
  # reorder batches
  batch <- factor(batch,
                  levels = levels(batch)[order(batchtime_first,
                                               batchtime_duration)])
